...
// NDFC
// Solve kernels with SHUFFLE!!

template<typename Float>
__device__ void solve_lower_with_shuffle(int nrhs, int nrows, int *stack_id, int *levels, volatile bool *solved_rows,
                                         int *rows, int *columns, Float *values, volatile Float *x, Float *b, int *perm,
                                         int *data_shuffle) {
  ...
  int row_start = rows[row];
  int row_end = rows[row + 1] - 1;
  Float diag_entry = values[ data_shuffle[row_end] ];
  Float r;
  if (thread_idx < nrhs)
    r = b[perm[row] * nrhs + thread_idx];
  int col;
  ...
  for (int i=row_start; i<row_end; ++i) {
    ...
    if (cache_idx == 0) {
      ...
      if (i + thread_idx < row_end) {
        cols_cache[thread_idx] = columns[i + thread_idx];
        vals_cache[thread_idx] = values[ data_shuffle[i + thread_idx] ];
      }
      __syncthreads();
    }
...
template<typename Float>
__device__ void solve_upper_with_shuffle(int nrhs, int nrows, int *stack_id, int *levels, volatile bool *solved_rows,
                                         int *rows, int *columns, Float *values, volatile Float *x, Float *b, int *perm,
                                         int *data_shuffle) {
  ...
  int row_start = rows[row];
  int row_end = rows[row + 1] - 1;
  Float diag_entry = values[ data_shuffle[row_start] ];
  Float r;
  if (thread_idx < nrhs)
    r = x[row * nrhs + thread_idx];
  int col;
...
extern "C" __global__ void solve_lower_with_shuffle_float(int nrhs, int nrows, int *stack_id,  int *levels, bool *solved_rows, int *rows, int *columns, float *values, float *x, float *b, int *perm, int *data_shuffle) {
  solve_lower_with_shuffle<float>(nrhs, nrows, stack_id, levels, solved_rows, rows, columns, values, x, b, perm, data_shuffle);
}

extern "C" __global__ void solve_lower_with_shuffle_double(int nrhs, int nrows, int *stack_id, int *levels, bool *solved_rows, int *rows, int *columns, double *values, double *x, double *b, int *perm, int *data_shuffle) {
  solve_lower_with_shuffle<double>(nrhs, nrows, stack_id, levels, solved_rows, rows, columns, values, x, b, perm, data_shuffle);
}

extern "C" __global__ void solve_upper_with_shuffle_float(int nrhs, int nrows, int *stack_id, int *levels, bool *solved_rows, int *rows, int *columns, float *values, float *x, float *b, int *perm, int *data_shuffle) {
  solve_upper_with_shuffle<float>(nrhs, nrows, stack_id, levels, solved_rows, rows, columns, values, x, b, perm, data_shuffle);
}

extern "C" __global__ void solve_upper_with_shuffle_double(int nrhs, int nrows, int *stack_id, int *levels, bool *solved_rows, int *rows, int *columns, double *values, double *x, double *b, int *perm, int *data_shuffle) {
  solve_upper_with_shuffle<double>(nrhs, nrows, stack_id, levels, solved_rows, rows, columns, values, x, b, perm, data_shuffle);
}
